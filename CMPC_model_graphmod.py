...
class LSTM_model(object):
    ...
    def build_spa_graph(self, spa_graph, words_feat, spatial, words_parse, level=""):
        printer = tf.Print(1,["Here:"])
        printer = tf.Print(1,[tf.shape(spa_graph)], message= "The shape of orig graph:", summarize=10) * printer
        ...
        spa_graph_trans2 = self._conv("spa_graph_trans2_{}".format(level), spa_graph, 1, self.v_emb_dim, self.v_emb_dim,
                                     [1, 1, 1, 1])
        printer = tf.Print(1,[tf.shape(spa_graph_trans2)], message= "The shape of trans2 before:", summarize=10) * printer
        spa_graph_trans2 = tf.reshape(spa_graph_trans2, [self.batch_size, self.vf_h * self.vf_w, self.v_emb_dim])
        printer = tf.Print(1,[tf.shape(spa_graph_trans2)], message= "The shape of trans2:", summarize=10) * printer
        ...
        if self.pred is not None:
            ...
            pred_mat = tf.repeat(pred_arr, [1600], axis = 2)
            ...
            with tf.variable_scope("pred_adj", reuse=tf.AUTO_REUSE):
                ...
                printer = tf.Print(1,[tf.shape(pred_adj_mat)], message= "The shape of pred_mat:", summarize=10) * printer
                adj_mat = tf.reshape(a, [1, self.vf_h * self.vf_w, self.vf_h * self.vf_w])

        spa_graph_nodes_num = self.vf_h * self.vf_w * printer

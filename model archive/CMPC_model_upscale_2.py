...
class LSTM_model(object):
    ...
    def build_graph(self):
        ...
        # Stack the multi-level scores and the scaled fused features to create a new feature map
        conv_up_1 = tf.image.resize_bilinear(pred, score_c3.shape)
        conv_up_1 = self._conv("conv_up_1", tf.stack([conv_up_1, score_c3], axis=2), 3, 2, 1, [1,1,1,1])
        conv_up_1 = tf.nn.l2_normalize(conv_up_1)
        conv_up_1 = tf.nn.relu(conv_up_1)

        conv_up_2 = tf.image.resize_bilinear(conv_up_1, score_c4.shape)
        conv_up_2 = self._conv("conv_up_2", tf.stack([conv_up_2, score_c4], axis=2), 3, 2, 1, [1,1,1,1])
        conv_up_2 = tf.nn.l2_normalize(conv_up_2)
        conv_up_2 = tf.nn.relu(conv_up_2)

        conv_up_3 = tf.image.resize_bilinear(conv_up_2, score_c5.shape)
        conv_up_3 = self._conv("conv_up_2", tf.stack([conv_up_3, score_c5], axis=2), 3, 2, 1, [1,1,1,1])

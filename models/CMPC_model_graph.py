...
class LSTM_model(object):
    ...
    def build_graph(self):
        ...
        # If there are refinement iterations
        for i in range(0,self.refine_steps):

            refine_c5 = self.build_lang2vis_refine(self.pred, visual_feat_c5, words_feat, lang_feat,
                                        words_parse, spatial, level="refine_c5")
            refine_score_c5 = self._conv("refine_score_c5", refine_c5, 3, self.mlp_dim, 1, [1, 1, 1, 1])
            refine_c4 = self.build_lang2vis_refine(self.pred, visual_feat_c4, words_feat, lang_feat,
                                        words_parse, spatial, level="refine_c4")
            refine_score_c4 = self._conv("refine_score_c4", refine_c4, 3, self.mlp_dim, 1, [1, 1, 1, 1])
            refine_c3 = self.build_lang2vis_refine(self.pred, visual_feat_c3, words_feat, lang_feat,
                                        words_parse, spatial, level="refine_c3")
            refine_score_c3 = self._conv("refine_score_c3", refine_c3, 3, self.mlp_dim, 1, [1, 1, 1, 1])
            # fused_refine_feats = self.gated_exchange_fusion_lstm_2times(refine_c3,
            #                                                  refine_c4, refine_c5, valid_lang)
            fused_refine_feats = tf.concat([refine_score_c5, refine_score_c4, refine_score_c3], -1)
            refine_score = self._conv("refine_score", fused_refine_feats, 3, 3, 1, [1, 1, 1, 1])
            #refine_score = self._conv("refine_score", fused_refine_feats, 3, self.mlp_dim, 1, [1, 1, 1, 1])

            self.pred_refine = refine_score
            self.pred = self.pred + refine_score
    ...
    def build_lang2vis_refine(self, pred_mask, visual_feat, words_feat, lang_feat, words_parse, spatial, level=""):
        valid_lang_feat = self.valid_lang(words_parse, words_feat)
        vis_la_sp = self.mutan_fusion(valid_lang_feat, spatial, visual_feat, level=level)
        print("Build MutanFusion Module to get multi-modal features.")
        spa_graph_feat = self.build_refinement_graph(pred_mask, vis_la_sp, words_feat, spatial,
                                              words_parse, level=level)
        print("Build Lang2Vis Refinement Module.")
    ...
    def build_spa_graph(self, spa_graph, words_feat, spatial, words_parse, level=""):
        ...
        self.adjmatbefore = adj_mat
        self.spagraphbefore = spa_graph
        ...
        self.spagraphmid = spa_graph
        return spa_graph

    
    def build_refinement_graph(self, pred_mask, spa_graph, words_feat, spatial, words_parse, level=""):
        # Fuse visual_feat, lang_attn_feat and spatial for SGR
        words_trans = self._conv("words_trans_{}".format(level), words_feat, 1, self.rnn_size, self.rnn_size,
                                 [1, 1, 1, 1])
        words_trans = tf.reshape(words_trans, [self.batch_size, self.num_steps - self.valid_idx[0, 0], self.rnn_size])
        ...
        gw_affi_w = tf.nn.softmax(graph_words_affi, axis=2)
        gw_affi_v = tf.nn.softmax(graph_words_affi, axis=1)
        adj_mat_base = tf.matmul(gw_affi_w, gw_affi_v, transpose_b=True)
        # adj_mat: [B, HW, HW], sum == 1 on axis 2

        # Build an affinity matrix using prediction mask
        ...
        pred_affi_w = tf.nn.softmax(pred_mat, axis=2)
        pred_affi_v = tf.nn.softmax(pred_mat, axis=1)
        adj_mat_pred = tf.matmul(pred_affi_w, pred_affi_v, transpose_b=True)
        
        # Stack the two matrices and convolve to merge
        adj_mat = tf.stack([adj_mat_base, adj_mat_pred], axis=3)
        adj_mat = self._conv("pred_adj", adj_mat, 1, 2, 1, [1, 1, 1, 1])
        adj_mat = tf.reshape(adj_mat, [1, self.vf_h * self.vf_w, self.vf_h * self.vf_w])
        
        self.adjmatafter = adj_mat
        
        # Apply graph convolution on the spatial graph
        refine_graph_nodes_num = self.vf_h * self.vf_w
        refine_graph = tf.reshape(spa_graph, [self.batch_size, 1, refine_graph_nodes_num, self.v_emb_dim])
        refine_graph = self.graph_conv(refine_graph, refine_graph_nodes_num, self.v_emb_dim, adj_mat,
                                    graph_name="refine_graph", level=level)
        refine_graph = tf.reshape(refine_graph, [self.batch_size, self.vf_h, self.vf_w, self.v_emb_dim])
        refine_graph = tf.nn.l2_normalize(refine_graph, 3)
        self.spagraphafter = refine_graph
        return refine_graph
    ...
    def train_op(self):
        ...
        self.cls_loss_all = 0.5 * self.cls_loss + 0.7 * self.cls_loss_base + 0.1 * self.cls_loss_c5 \

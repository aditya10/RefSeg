...
class LSTM_model(object):
    ...
    def build_spa_graph(self, spa_graph, words_feat, spatial, words_parse, level=""):
        ...
        # word_trans is (1, len of expression = 5, 1000)
        ...
        # spa_graph_trans2 is (1,1600,1000)
        # (1600,1000)*(1000,5) = (1600,5)
        ...
        # graph_words_affi is (1, 1600, 5)
        gw_affi_w = tf.nn.softmax(graph_words_affi, axis=2)
        gw_affi_v = tf.nn.softmax(graph_words_affi, axis=1)
        self.wordsinfo = gw_affi_v
        adj_mat = tf.matmul(gw_affi_w, gw_affi_v, transpose_b=False)
        ...
        if self.pred_base is not None:
            ...
            adj_mat_pred = tf.matmul(pred_affi_w, pred_affi_v, transpose_b=False)
        ...
        if self.pred_base is not None:
            # spa_graph = tf.math.negative(spa_graph)
            self.spagraphafter = spa_graph
        else:

...
class LSTM_model(object):
    ...
    def build_graph(self):
        ...
        fused_feats = self.gated_exchange_fusion_lstm_2times(fusion_c3,
                                                            fusion_c4, fusion_c5, valid_lang)
        score_graph = self._conv("score_2", fused_feats, 3, self.mlp_dim, 1, [1, 1, 1, 1])
        self.score_graph = score_graph
        self.pred_graph = self.pred_base + score_graph
        self.up_graph = tf.image.resize_bilinear(self.pred_graph, [self.H, self.W])
        self.sigm_graph = tf.sigmoid(self.up_graph)

        ########################
        # REFINEMENT
        ########################
        upscore = tf.image.resize_bilinear(self.pred_graph, [224, 224])
        ...
        self.score_refine = self._conv("predrf", l1, 3, 64, 1, [1, 1, 1, 1])

        # Final
        self.pred = upscore + self.score_refine
    ...
    def build_spa_graph(self, spa_graph, words_feat, spatial, words_parse, level=""):
        ...
        if self.pred_base is not None:
            ...
            pref_affi = tf.matmul(pred_mat, pred_mat, transpose_b=True)
            pred_affi_w = tf.nn.softmax(pref_affi, axis=2)
            pred_affi_v = tf.nn.softmax(pref_affi, axis=1)
            adj_mat_pred = tf.matmul(pred_affi_w, pred_affi_v, transpose_b=True)
            new_adj_mat = tf.stack([adj_mat, adj_mat_pred], axis=3)
            self.adj_pred = adj_mat_pred
    ...
    def train_op(self):
        ...
        # Graph Loss
        self.cls_loss_graph = loss.weighed_logistic_loss(self.up_graph, self.target_fine, 1, 1)
        # Final Loss
        ...
        # All Losses
        self.cls_loss_all = 0.1 * self.cls_loss_c5 \
                            + 0.1 * self.cls_loss_c4 \
                            + 0.1 * self.cls_loss_c3 \
                            + 1 * self.cls_loss_base \
                            + 0.5 * self.cls_loss_graph \
                            + 1 * self.cls_loss \
                            + 0.5 * self.iou_loss

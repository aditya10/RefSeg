...
def train(max_iter, snapshot, dataset, setname, mu, lr, bs,
          conv5, model_name, stop_iter, pre_emb=False, test=False, visualize=False):
    ...
    model_folder = '/ubc/cs/research/shield/projects/aditya10/RefSeg/results'+model_name
    model_dataset_folder = model_folder+'/'+dataset
    train_folder = model_dataset_folder+'/train/'
    
    if not os.path.isdir(train_folder):
        os.makedirs(train_folder)
    
    snapshot_file = os.path.join(train_folder, dataset + '_iter_%d.tfmodel')
    ...
    for n_iter in range(max_iter):
        ...
        if n_iter % iters_per_log == 0:
            ...
            with open(train_folder+"avgloss.txt", "a") as myfile:
                myfile.write("%f\n"%(cls_loss_avg))
            with open(train_folder+"avgacc.txt", "a") as myfile:
                myfile.write("%f\n"%(avg_accuracy_all))
    ...
    if test:
        print('Starting tests on validation set')
        test(stop_iter, dataset, visualize, 'val', True, mu, model_name, pre_emb=pre_emb)


def test(iter, dataset, visualize, setname, dcrf, mu, model_name, pre_emb=False):
    ...
    model_dataset_folder = model_folder+'/'+dataset
    train_folder = model_dataset_folder+'/train'
    test_folder = model_dataset_folder+'/test'+str(iter)+'/'
    
    if not os.path.isdir(test_folder):
        os.makedirs(test_folder)

    if visualize:
        save_dir = test_folder+'visualize/'
        if not os.path.isdir(save_dir):
            os.makedirs(save_dir)
        
    weights = os.path.join(train_folder, dataset + '_iter_' + str(iter) + '.tfmodel')
    ...
    for n_iter in range(reader.num_batch):
        ...
        if visualize: 
            ...
            adj_base,\
            adj_pred,\
            adj_refine = sess.run([model.pred, 
                                    model.up, 
                                    model.sigm, 
                                    model.pred_base, 
                                    model.up_base, 
...
# -d Gref 
# -t train 
# -n CMPC_model_graphmod_dup_loss_2 
# -emb 
# -f /ubc/cs/research/shield/projects/aditya10/RefSeg/ckpts/Gref/15
...
if __name__ == "__main__":
    ...
    # For training
    parser.add_argument('-g', type=str, default='0') # CUDA, unused
    parser.add_argument('-i', type=int, default=700000) # max iterations
    parser.add_argument('-s', type=int, default=100000) # snapshot after s iterations
    ...
    parser.add_argument('-test', default=False, action='store_true') # autorun tests
